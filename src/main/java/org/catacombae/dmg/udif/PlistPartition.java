...
public class PlistPartition {
    ...
    public PlistPartition(String name, String id, String attributes, InputStream data, long previousOutOffset, long previousInOffset)
            throws IOException {
        this.name = name;
        this.id = id;
    ...
    /**
     * Returns an iterator over all the UDIFBlocks that describe the contents of
     * this partition.
     */
    ...
    private UDIFBlock[] parseBlocks(InputStream is) throws IOException {
        ...
        int blockNumber = 0; // Increments by one for each block we read (each iteration in the while loop
                             // below)
        ...
        while (bytesRead > 0) { // offset <= data.length-UDIFBlock) {
            ...
            if (bytesRead != blockData.length)
                throw new RuntimeException(
                        "Could not read the desired amount of bytes... (desired: " + blockData.length + " read: " + bytesRead + ")");

            long inOffset = UDIFBlock.peekInOffset(blockData, 0);
            long inSize = UDIFBlock.peekInSize(blockData, 0);

            // Set compensation to the end of the output data of the previous partition to
            // get true offset in outfile.
            ...
            /*
             * The lines below are a "hack" that I had to do to make dmgx work with certain
             * dmg-files. I don't understand the issue at all, which is why this hack is
             * here, but sometimes inOffset == 0 means that it is 0 relative to the previous
             * partition's last inOffset. And sometimes it doesn't (meaning the actual
             * position 0 in the dmg file).
             */
            ...
            if (addInOffset) {
                Debug.notification("addInOffset mode: inOffset tranformation " + inOffset + "->" + (inOffset + previousInOffset));
