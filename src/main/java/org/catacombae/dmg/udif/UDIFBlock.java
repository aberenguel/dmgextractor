...
public class UDIFBlock implements Comparable<UDIFBlock> {
    /**
     * This blocktype means the data is compressed using some "ADC" algorithm that I
     * have no idea how to decompress...
     */
    ...
    /**
     * This blocktype means the data is compressed with the bzip2 compression
     * algorithm. These blocktypes are unsupported, as I haven't found a
     * GPL-compatible bzip2 decompressor written in Java yet.
     */
    ...
    /**
     * This blocktype represents a fill of zeroes (the difference between this
     * blocktype and BT_ZERO is not documented, and parsing of these blocks is
     * experimental).
     */
    ...
    /**
     * This blocktype has been observed, but its purpose is currently unknown. In
     * all the observed cases the outSize was equal to 0, so it's probably some
     * marker, like BT_END.
     */
    ...
    /*
     * BP Name Size ------------------- 0 blockType 4 4 skipped 4 8 outOffset 8 16
     * outSize 8 24 inOffset 8 32 inSize 8 ------------------- 40 bytes / 0x28 bytes
     */
    ...
    public UDIFBlock(byte[] data, int offset, long outOffsetComp, long inOffsetComp) {
        this(org.catacombae.util.Util.readIntBE(data, offset + 0), org.catacombae.util.Util.readIntBE(data, offset + 4),
                org.catacombae.util.Util.readLongBE(data, offset + 8) * 0x200, org.catacombae.util.Util.readLongBE(data, offset + 16) * 0x200,
                org.catacombae.util.Util.readLongBE(data, offset + 24), org.catacombae.util.Util.readLongBE(data, offset + 32), outOffsetComp,
                inOffsetComp);
    }
    ...
    /**
     * Convenience method for determining the actual compensated out offset. This is
     * what you should use.
     */
    ...
    /**
     * Convenience method for determining the actual compensated in offset. This is
     * what you should use.
     */
    ...
    public String toString() {
        return getBlockTypeAsString() + "(reserved=0x" + Integer.toHexString(reserved) + ",outOffset=" + outOffset + ",outSize=" + outSize
                + ",inOffset=" + inOffset + ",inSize=" + inSize + ",outOffsetComp=" + outOffsetComp + ",inOffsetComp=" + inOffsetComp + ")";
    }
    ...
    public static long peekInOffset(byte[] data, int offset) {
        return org.catacombae.util.Util.readLongBE(data, offset + 24);
    }

    /**
     * Reads the inSize field from <code>data</code> at <code>offset</code> which is
     * supposed to be a valid raw UDIF block structure at 40 bytes.
     */
    public static long peekInSize(byte[] data, int offset) {
        return org.catacombae.util.Util.readLongBE(data, offset + 32);
    }
