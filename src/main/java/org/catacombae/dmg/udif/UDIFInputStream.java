...
/**
 * An InputStream for reading the "block device" contents of a UDIF disk image
 * file, usually using the extension .dmg. Note that all files with the
 * extension .dmg are not UDIF encoded. Some are raw disk images, with no
 * wrapper, and can be burnt or restored directly to disk/optical disc.<br>
 * Make sure that no other stream concurrently uses the input file. This class
 * isn't thread safe, so use external synchronization if needed.
 */
public class UDIFInputStream extends InputStream {
    ...
    /**
     * Constructs a new UDIF input stream from a RandomAccessFile. This is a
     * convenience method, and equal to <code>new UDI
     */
    public UDIFInputStream(RandomAccessFile raf, String openPath) throws IOException {
    ...
    /**
     * Returns the number of available bytes, or Integer.MAX_VALUE if the value is
     * too large for int.
     */
    @Override
    public int available() throws IOException {
        ...
        if (len > Integer.MAX_VALUE)
            ...
            throw new IOException(
                    "Internal error! filePointer > wrapped.length! filePointer:" + filePointer + " wrapped.length():" + wrapped.length());
        else
            return (int) len;
    }
