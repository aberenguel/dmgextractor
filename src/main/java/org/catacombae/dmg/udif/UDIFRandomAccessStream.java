...
public class UDIFRandomAccessStream extends BasicReadableRandomAccessStream {
    /*
     * We have a string of data divided into blocks. Different algorithms must be
     * applied to different types of blocks in order to extract the data.
     */
    ...
    private long length;
    /**
     * This is the pointer to the current position in the virtual file provided by
     * this stream.
     */
    ...
    public UDIFRandomAccessStream(UDIFFile dmgFile) throws RuntimeIOException {
        ...
        // dbg("dmgFile.getView().getPlist(); free memory: " +
        // Runtime.getRuntime().freeMemory() + " total memory: " +
        // Runtime.getRuntime().totalMemory());
        Plist plist = dmgFile.getView().getPlist();
        // dbg("before gc(): free memory: " + Runtime.getRuntime().freeMemory() + "
        // total memory: " + Runtime.getRuntime().totalMemory());
        // Runtime.getRuntime().gc();
        // dbg("plist.getPartitions(); free memory: " +
        // Runtime.getRuntime().freeMemory() + " total memory: " +
        // Runtime.getRuntime().totalMemory());
        try {
    ...
    public int read(byte[] b, int off, int len) throws RuntimeIOException {
        try {
            // System.out.println("UDIFRandomAccessStream.read(b.length=" + b.length + ", "
            // + off + ", " + len + ") {");
            if (seekCalled) {
                seekCalled = false;
                // System.out.print(" Repositioning stream after seek (logical file pointer: " +
                // logicalFilePointer + ")...");
                try {
                    repositionStream();
...
// 	System.out.println("<UDIFRandomAccessStream.repositionStream()>");
        try {
            // if the global file pointer is not within the bounds of the current block,
            // then find the accurate block
            if (!(currentBlock.getTrueOutOffset() <= logicalFilePointer
                ...
                if (soughtBlock != null) {
                    // System.out.println("REPOSITION " + currentBlock.getBlockTypeAsString() + "("
                    // + currentBlock.getTrueOutOffset() + "," + currentBlock.getOutSize() + ") -> "
                    // + soughtBlock.getBlockTypeAsString() + "(" + soughtBlock.getTrueOutOffset() +
                    // "," + soughtBlock.getOutSize() + ")");
...
// 	System.out.println("</UDIFRandomAccessStream.repositionStream()>");
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (args.length != 2)
            ...
            UDIFRandomAccessStream dras = new UDIFRandomAccessStream(
                    new UDIFFile(new ReadableFileStream(new RandomAccessFile(args[0], "r"), args[0])));
