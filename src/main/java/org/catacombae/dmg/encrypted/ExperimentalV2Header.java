...
public class ExperimentalV2Header {
    /*
     * struct ExperimentalV2Header size: 248 bytes description:
     *
     * BP Size Type Identifier Description
     * -----------------------------------------------------------------------------
     * ------------------------------- 0 1*8 uint8_t[8] signature Header signature
     * (ASCII: 'encrcdsa'). 8 4 uint32_t possibleHeaderVersion Possibly the version
     * of the encrypted volume format. 12 4 uint32_t laban Unknown variable with
     * observed value 16/0x10. 16 4 uint32_t edward Unknown variable with observed
     * value 5/0x5. 20 4 uint32_t palle Unknown variable with observed value
     * 0x80000001. 24 4 uint32_t lisa Unknown variable with observed value 128/0x80.
     * 28 4 uint32_t unknownInt28 Unknown variable with observed value 91/0x5B. 32 4
     * uint32_t unknownInt32 Unknown variable with observed value 160/0xA0. 36 1*16
     * uint8_t[16] unknown1 Unknown binary data. 52 4 uint32_t blockSize Block size
     * of the encrypted block data. 56 8 uint64_t encryptedDataLength Length in
     * bytes of the data that has been encrypted. 64 8 uint64_t offsetToDataStart
     * Offset to the start of the encrypted block data. 72 4 uint32_t unknownInt72
     * Unknown variable with observed value 1/0x1. 76 4 uint32_t unknownInt76
     * Unknown variable with observed value 1/0x1. 80 8 uint64_t
     * possiblePointerToKdfAlgorithm Could be a pointer to where kdf_algorithm is
     * located. 88 8 uint64_t unknownLong88 Unknown variable with observed value
     * 616/0x268. 96 4 uint32_t kdfAlgorithm Algorithm of the key derivation
     * function. 100 4 uint32_t kdfPrngAlgorithm ? 104 4 uint32_t kdfIterationCount
     * Iteration count (normally 1000). 108 4 uint32_t kdfSaltLen Length of kdfSalt
     * (in bytes). 112 1*32 uint8_t[32] kdfSalt Salt value for key derivation. 144 4
     * uint32_t blobEncIvSize Size of blobEncIv. 148 1*32 uint8_t[32] blobEncIv
     * Initialization Vector for encryption-key unwrapping. 180 4 uint32_t
     * blobEncKeyBits Number of bits in the keyblob's encryption key. 184 4 uint32_t
     * blobEncAlgorithm Encryption algorithm used to encrypt the key blob. 188 4
     * uint32_t blobEncPadding Padding. (?) 192 4 uint32_t blobEncMode Encryption
     * mode for the algorithm. 196 4 uint32_t encryptedKeyblobSize Size of
     * encryptedKeyBlob. 200 1*48 uint8_t[48] encryptedKeyblob The encrypted key
     * blob, containing all keys.
     */
    ...
    public byte[] getUnknown1() {
        return org.catacombae.util.Util.readByteArrayBE(unknown1);
    }
    ...
    public int getUnknownInt72() {
        return org.catacombae.util.Util.readIntBE(unknownInt72);
    }

    /** Unknown variable with observed value 1/0x1. */
    public int getUnknownInt76() {
        return org.catacombae.util.Util.readIntBE(unknownInt76);
    }

    /** Could be a pointer to where kdf_algorithm is located. */
    public long getPossiblePointerToKdfAlgorithm() {
        return org.catacombae.util.Util.readLongBE(possiblePointerToKdfAlgorithm);
    }

    /** Unknown variable with observed value 616/0x268. */
    public long getUnknownLong88() {
        return org.catacombae.util.Util.readLongBE(unknownLong88);
    }
