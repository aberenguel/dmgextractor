...
public class SparseImageHeader {
    /*
     * struct SparseImageHeader size: 4096 bytes description:
     *
     * BP Size Type Identifier Description
     * -----------------------------------------------------------------------------
     * -----------------------------------------------------------------------------
     * ------------------------------------------------------------------ 0 1 * 4
     * u8[4] signature Must contain the string "sprs". 4 4 be32 reservedAt4 Possibly
     * the sparseimage format version (observed value: 3). 8 4 be32 sectorsPerBlock
     * The number of sectors in each sparseimage block (observed value: 2048). 12 4
     * be32 reservedAt12 Changes from 0x00000000 to 0x00000001 when image goes from
     * empty -> containing content. Stays at 0x00000001 as new blocks are allocated.
     * Likely to be a "flags" field of some kind. 16 4 be32 sectorCount The number
     * of sectors in the virtual device. 20 1 * 12 u8[12] reservedAt20 Observed
     * value: 0x0 for all bytes. 32 4 be32 sectorCount2 The number of sectors in the
     * virtual device (again). One of these two fields has some additional meaning.
     * 36 1 * 28 u8[28] reservedAt36 Observed value: 0x0 for all bytes. 64 4 * 1008
     * be32[1008] blockMap The block map, mapping real to virtual block indices.
     */
     ...
     * containing content. Stays at 0x00000001 as new blocks are allocated. Likely
     * to be a "flags" field of some kind.
     */
    ...
    /**
     * The number of sectors in the virtual device (again). One of these two fields
    ...
    /**
     * <b>Note that the return value from this function should be interpreted as an
     * unsigned integer, for instance using Util.unsign(...).</b>
     */
    ...
    public void printFields(PrintStream ps, String prefix) {
        ps.println(prefix + " signature: \"" + Util.readString(getSignature(), "US-ASCII") + "\"");
