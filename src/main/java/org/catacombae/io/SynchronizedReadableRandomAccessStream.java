...
public class SynchronizedReadableRandomAccessStream extends BasicSynchronizedReadableRandomAccessStream implements SynchronizedReadableRandomAccess {
    ...
    private static final boolean DEBUG = Util.booleanEnabledByProperties(false, "org.catacombae.debug", "org.catacombae.io.debug",
            "org.catacombae.io." + SynchronizedReadableRandomAccessStream.class.getSimpleName() + ".debug");
    private static final boolean REFERENCES_DEBUG = Util.booleanEnabledByProperties(DEBUG,
            "org.catacombae.io." + SynchronizedReadableRandomAccessStream.class.getSimpleName() + ".references_debug");
    ...
    public SynchronizedReadableRandomAccessStream(ReadableRandomAccessStream sourceStream) {
        ...
        if (REFERENCES_DEBUG) {
            references.put(this, new Reference(this, new Exception().getStackTrace()));
        }
    }
    ...
    public synchronized void close() throws RuntimeIOException {
        if (DEBUG) {
            System.err.println(SynchronizedReadableRandomAccessStream.class.getName() + "@" + Util.toHexStringBE(hashCode()) + ".close(): Called "
    ...
    public synchronized void addReference(Object referrer) {
        if (DEBUG) {
            System.err.println(this + ": Reference added (" + refCount + " " + "-> " + (refCount + 1) + ") by " + referrer + ".");
        }
    ...
    public synchronized void removeReference(Object referrer) {
        ...
        if (DEBUG) {
            System.err.println(this + ": Reference removed (" + refCount + " " + "-> " + (refCount - 1) + ") by " + referrer + ".");
        }
    ...
    public synchronized void finalize() throws Throwable {
        try {
            if (refCount != 0) {
                System.err.println("[WARNING] " + this + " is garbage " + "collected with " + refCount + " remaining references"
                        + (REFERENCES_DEBUG ? ":" : "."));
                if (REFERENCES_DEBUG) {
