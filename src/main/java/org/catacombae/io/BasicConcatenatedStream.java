...
public abstract class BasicConcatenatedStream<A extends ReadableRandomAccessStream> extends BasicReadableRandomAccessStream {
    ...
    private static final IOLog log = IOLog.getInstance(BasicConcatenatedStream.class);
    ...
    protected BasicConcatenatedStream(A firstPart, long startOffset, long length) {
        ...
        try {
            if (startOffset < 0) {
                /*
                 * Negative startOffset means there is an hole segment inserted before the first
                 * byte of the stream.
                 */
    ...
    private void enter(String methodName, Object... args) {
        ...
        out.print(this.getClass().getSimpleName() + "{" + this.hashCode() + "}");
        if (methodName != null)
    ...
    private void log(String methodName, String message) {
        PrintStream out = System.err;
        out.println(this.getClass().getSimpleName() + "{" + this.hashCode() + "}." + methodName + ": " + message);
    }
        ...
        try {
            ...
            while (requestedPartIndex < parts.size()) {
                ...
                if (log.debug) {
                    ...
                    log.debug("requestedPart.startOffset = " + requestedPart.startOffset);
                }
                ...
                if (requestedPart.file == null) {
                    ...
                    if (log.debug) {
                        log.debug("invoking requestedPart.file.read(byte[" + data.length + "], " + (off + bytesRead) + ", " + bytesToRead + ")");
                    }
