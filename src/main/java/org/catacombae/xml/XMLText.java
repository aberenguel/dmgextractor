...
public class XMLText extends XMLElement {
    ...
    public XMLText(SynchronizedRandomAccessStream xmlFile, Charset encoding, long beginOffset, long endOffset) {
        ...
        // SynchronizedRandomAccessStream sras = new SynchronizedRandomAccessStream(new
        // RandomAccessFileStream(raf));
    }

    /**
     * This way of dealing with the issue of lines and columns is very heavy and
     * fragile as it tries to read through the entire xmlFile to seek forward to the
     * desired lines and columns (there's no algorithmic way to do this, so we have
     * toseek exhaustively). The calculation will not be performed in the
     * constructor, but on the first call to getText()... (in order to reduce
     * workload)
     */
        ...
        while (currentChar >= 0) {
            ...
            if (c == CR) {
                ...
                if (!previousCR) {
                    ...
                    previousCR = false;
                    lfskip = true; // We haven't changed the col or line number in this iteration, as in the other
                                   // cases
                }
            } else if (c == TAB) {
                colNumber += 8;
...
// 	    System.err.println("Trying to read... lineNumber=" + lineNumber + " colNumber=" + colNumber);
        ...
        if (beginOffset == -1 || endOffset == -1)
            throw new RuntimeException("Could not find the requested interval! (begin: (" + beginLine + "," + beginColumn + ") end: (" + endLine + ","
                    + endColumn + "))");
// 	else
