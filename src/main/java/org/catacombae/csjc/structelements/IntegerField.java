...
public class IntegerField extends StringRepresentableField {
    ...
    public IntegerField(Object obj, Field fieldData, int offset, IntegerFieldBits bits, Signedness signedness, Endianness endianness,
            IntegerFieldRepresentation representation, String unitComponent) {
    ...
    public IntegerField(DataHandle fieldData, int offset, IntegerFieldBits bits, Signedness signedness, Endianness endianness,
            IntegerFieldRepresentation representation, String unitComponent) {
    ...
    public String getValueAsString() {
        return representation.getPrefix() + getValueAsBigInteger().toString(representation.getRadix());
    }
        ...
        if (validateMsg == null) {
            ...
            if (signedness == Signedness.UNSIGNED && ba.length != (bits.getBytes() + 1))
                throw new RuntimeException(
                        "UNEXPECTED: ba.length (" + ba.length + ") != bits.getBytes()+1(" + bits.getBytes() + "+1=" + (bits.getBytes() + 1) + ")");
            byte[] trueContents;
            if (endianness == Endianness.LITTLE_ENDIAN)
